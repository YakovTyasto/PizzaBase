-- Version a recipe on every real change, and never twice for the same state.
--
-- Root cause of the "experiments need two versions" dead end: `save_recipe`
-- only snapshotted a recipe whose status was already `verified`. Nothing in the
-- seeded catalog is verified, so no amount of editing ever produced a single
-- historical version, and the Experiments screen had nothing to put beside the
-- current recipe. It listed every recipe and told the owner the same thing
-- about all of them.
--
-- Two helpers make the new rule expressible: one builds the snapshot document,
-- the other strips the fields that move on their own so two snapshots can be
-- compared for genuine equality.

-- ---------------------------------------------------------------------------
-- Snapshot helpers
-- ---------------------------------------------------------------------------

create or replace function build_recipe_snapshot(p_recipe_id uuid)
returns jsonb
language sql
stable
security invoker
...
as $$
  select jsonb_build_object(
    'recipe', (select to_jsonb(r) from recipes r where r.id = p_recipe_id),
    'translations', (
      select coalesce(jsonb_agg(to_jsonb(t) order by t.locale), '[]'::jsonb)
      from recipe_translations t where t.recipe_id = p_recipe_id
    ),
    'items', (
      select coalesce(jsonb_agg(to_jsonb(i) order by i.sort_order, i.id), '[]'::jsonb)
      from recipe_items i where i.recipe_id = p_recipe_id
    ),
    'steps', (
      select coalesce(jsonb_agg(to_jsonb(s) order by s.sort_order, s.id), '[]'::jsonb)
      from recipe_steps s where s.recipe_id = p_recipe_id
    ),
    'step_translations', (
      select coalesce(jsonb_agg(to_jsonb(st) order by st.step_id, st.locale), '[]'::jsonb)
...
comment on function build_recipe_snapshot(uuid) is
  'The immutable document stored in recipe_versions.snapshot.';

-- Everything that identifies a *state* rather than a *write*. Row ids and the
-- timestamps the database maintains itself are removed, so saving an unchanged
-- recipe compares equal to what is already stored.
create or replace function comparable_recipe_snapshot(p_snapshot jsonb)
returns jsonb
language sql
immutable
security invoker
...
as $$
  select jsonb_build_object(
    'recipe', (p_snapshot -> 'recipe') - 'updated_at' - 'created_at',
    'translations', p_snapshot -> 'translations',
    'items', (
      select coalesce(jsonb_agg((value - 'id') - 'created_at'), '[]'::jsonb)
      from jsonb_array_elements(coalesce(p_snapshot -> 'items', '[]'::jsonb))
    ),
    'steps', (
      select coalesce(jsonb_agg((value - 'id') - 'created_at'), '[]'::jsonb)
      from jsonb_array_elements(coalesce(p_snapshot -> 'steps', '[]'::jsonb))
    ),
    'step_translations', (
      select coalesce(jsonb_agg(value - 'step_id'), '[]'::jsonb)
      from jsonb_array_elements(coalesce(p_snapshot -> 'step_translations', '[]'::jsonb))
    )
  );
$$;

comment on function comparable_recipe_snapshot(jsonb) is
  'A snapshot with self-maintained ids and timestamps removed, for equality.';

-- ---------------------------------------------------------------------------
-- save_recipe, with the new versioning rule
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Snapshot the recipe as it stands before overwriting it.
  --
  -- Every edit of an existing recipe is versioned, not only an edit of a
  -- `verified` one. Restricting it to `verified` meant a catalog where nothing
  -- is verified yet could never accumulate a single snapshot -- which is why
  -- the Experiments screen had nothing to compare and could only ever ask for
  -- a second version.
  --
  -- The comparison against the newest stored snapshot is what keeps that from
  -- filling the history with noise: if it already holds this exact state, the
  -- save changed nothing worth recording. `updated_at` is excluded because the
  -- row's own trigger moves it on every write, so leaving it in would make two
  -- identical saves look different. `p_create_version` is still accepted for
  -- compatibility but no longer forces a duplicate.
  if found then
    v_snapshot := build_recipe_snapshot(v_existing.id);

    select snapshot into v_previous_snapshot
    from recipe_versions
    where recipe_id = v_existing.id
    order by version_number desc
    limit 1;

    if v_previous_snapshot is null
       or comparable_recipe_snapshot(v_previous_snapshot)
            is distinct from comparable_recipe_snapshot(v_snapshot)
    then
      ...
      values (v_existing.id, v_owner, v_next_version, v_snapshot, p_draft ->> 'versionNote');
      v_versioned := true;
    end if;
  end if;
...
comment on function save_recipe(jsonb, boolean) is
  'Writes a whole recipe in one transaction, versioning any real change.';
