...
export async function RecipeCard({ recipe }: { recipe: RecipeSummary }) {
  ...
  return (
    <Card className="overflow-hidden transition-shadow hover:shadow-[var(--shadow-raised)]">
      <Link href={`/recipes/${recipe.slug}`} className="block">
        {/* The cover, when there is one. A recipe without a photo keeps the
            typographic card it always had rather than showing a grey box. */}
        {recipe.cover ? (
          <MediaImage
            id={recipe.cover.id}
            url={recipe.cover.url}
